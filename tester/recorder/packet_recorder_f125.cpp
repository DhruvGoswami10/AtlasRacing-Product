...
public:
    PacketRecorderF125() : socket_fd(-1), packet_count(0), recording(false) {}
    
    bool initialize(int port = 20777) {
        std::cout << "🎬 Atlas Racing F1 25 Packet Recorder" << std::endl;
        std::cout << "======================================" << std::endl;
    ...
    bool startRecording(const std::string& filename, const std::string& session_name = "") {
        ...
        // Write file header
        F125FileHeader header;
        memset(&header, 0, sizeof(header));
        strcpy(header.magic, "F125");
        header.version = 1;
        ...
        std::cout << "🔴 Recording started (F1 25): " << filename << std::endl;
        if (!session_name.empty()) {
    ...
    void recordLoop() {
        ...
        while (recording) {
            ...
            if (bytes_received > 0) {
                ...
                if (static_cast<size_t>(bytes_received) > F125_MAX_PACKET_SIZE) {
                    std::cerr << "[Recorder] Packet too large (" << bytes_received
                              << " > " << F125_MAX_PACKET_SIZE << "), skipping" << std::endl;
                    continue;
                }
                ...
                // Extract packet ID from F1 24/25 packet header
                // PacketHeader structure: packetFormat(2) + gameYear(1) + gameMajorVersion(1) +
                //                         gameMinorVersion(1) + packetVersion(1) + packetId(1) = offset 6
                uint8_t packet_id = 0;
                if (bytes_received >= 7) { // Need at least 7 bytes for packet ID
                    packet_id = buffer[6]; // Packet ID is at offset 6 in F1 24/25 header
                }
...
PacketRecorderF125* g_recorder = nullptr;
...
int main(int argc, char* argv[]) {
    ...
    std::string filename = "session_recording.f125";
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-f" || arg == "--file") {
            ...
            std::cout << "  -f, --file <filename>    Output file (default: session_recording.f125)" << std::endl;
    ...
    g_recorder = new PacketRecorderF125();
    
    if (!g_recorder->initialize(port)) {
        delete g_recorder;
        return 1;
    }
    
    if (!g_recorder->startRecording(filename, session_name)) {
        delete g_recorder;
        return 1;
    }
    
    g_recorder->recordLoop();
